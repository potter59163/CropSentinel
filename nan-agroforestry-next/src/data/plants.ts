...
export const HABIT_META: Record<Habit, { chip: string; note: string }> = {
  ...
  'ไม้เถา': {
    chip: 'ไม้เถา',
    note: 'เถาเนื้อแข็ง อายุหลายปี ปีนป่ายเกาะไม้ใหญ่หรือค้าง — หวายก็เป็นแบบนี้ ลำเหนียวจนตัดไปทำเครื่องจักสานได้ '
      + '· เบียดพื้นที่ดินน้อย แต่ขึ้นไปแย่งแสงชั้นบนได้',
  },
  'ไผ่': {
    chip: 'ไผ่ (หญ้า)',
  ...
  'พืชล้มลุก': {
    ...
    note: 'ลำต้นอวบน้ำ ไม่มีเนื้อไม้ ส่วนใหญ่ตายลงหลังให้ผลผลิตแล้วปลูกใหม่ '
      + '· ตำราพฤกษศาสตร์เรียก "ไม้ล้มลุก" ซึ่ง "ไม้" ในที่นี้แปลว่าพืช ไม่ได้แปลว่าเนื้อไม้',
  },
  'พืชล้มลุกขนาดใหญ่': {
    chip: 'พืชล้มลุกขนาดใหญ่',
    note: 'สูงเท่าต้นไม้แต่ไม่ใช่ต้นไม้ — กล้วยที่ "ลำต้น" จริง ๆ คือกาบใบซ้อนกันแน่น ไม่มีแก่น '
      + 'ตัดด้วยมีดพร้าได้ และตายทั้งต้นหลังตกเครือ แล้วหน่อข้างขึ้นแทน · มะละกอก็ไม่มีแก่นเช่นกัน',
  },
  'พืชล้มลุกมีเหง้า': {
    chip: 'พืชล้มลุกมีเหง้า',
    note: 'ต้นที่เห็นเหนือดินเป็นกาบใบซ้อนกัน ไม่ใช่ลำต้นจริง · ตัวพืชจริงคือเหง้าใต้ดิน ซึ่งอยู่ข้ามปีได้ '
      + 'ไม่ตายไปตามต้น จึงไม่ต้องปลูกใหม่ทุกปีเหมือนพืชล้มลุกทั่วไป — และเหง้านี่เองคือส่วนที่เก็บขาย',
  },
  'พืชล้มลุกมีหัว': {
    chip: 'พืชล้มลุกมีหัว',
    note: 'ไม่มีเนื้อไม้ สะสมอาหารไว้ในหัวใต้ดิน ต้นเหนือดินยุบลงเมื่อหมดฤดู แล้วแตกใหม่จากหัวเดิม',
  },
  'เถาล้มลุก': {
    chip: 'เถาล้มลุก',
    note: 'เถาอ่อนไม่มีเนื้อไม้ ทอดคลุมดินหรือเลื้อยขึ้นค้าง อายุสั้น ปลูกใหม่ทุกรอบ '
      + '· ต่างจาก "ไม้เถา" ที่เถาแข็งและอยู่ข้ามปี',
  },
  'หญ้า': {
    chip: 'หญ้า',
...
export const PLANTS: Plant[] = [
  ...
  P('coffee', 'กาแฟอาราบิก้า', 'Arabica coffee', 'shrub', 'ไม้พุ่ม', 'กาแฟ', 800, 1600, true, 3, 5, 150, 220, 0.7, 0.35, false, 'med', 9000, 6500, 1, 'coffee', 'อาราบิก้าควรอยู่พื้นที่สูงเย็น 800-1000+ ม. ชอบร่มเงาและน้ำสม่ำเสมอ'),
  P('chili', 'พริก', 'Chili', 'shrub', 'พืชล้มลุก', 'พืชผัก', 0, 1200, false, 1, 1, 20, 900, 0.3, 0.2, false, 'med', 3000, 3500, 2, 'chili', 'รายได้ดี เก็บได้หลายรอบ พื้นที่สูงเกินไปจะช้าลงและต้องการแดดพอควร'),
  ...
  P('tea', 'ชาเมี่ยง', 'Assam tea', 'shrub', 'ไม้ต้น', 'เครื่องดื่ม', 800, 1600, true, 4, 6, 60, 350, 0.6, 0.3, false, 'med', 7000, 7800, 2, undefined,'พืชใต้ร่มดั้งเดิมภาคเหนือ เหมาะภูเขาเย็นชื้น เก็บได้หลายรอบ'),
  ...
  P('peanut', 'ถั่วลิสง', 'Peanut', 'groundcover', 'พืชล้มลุก', 'พืชตระกูลถั่ว', 0, 1500, false, 1, 1, 35, 250, 0.35, 0, true, 'low', 1200, 5300, 2, 'peanut', 'ตรึงไนโตรเจนบำรุงดิน คลุมหน้าดิน เหมาะอากาศอุ่นและดินไม่แฉะ'),
  P('pumpkin', 'ฟักทอง', 'Pumpkin', 'groundcover', 'เถาล้มลุก', 'พืชเลื้อย', 0, 1200, false, 1, 1, 14, 1800, 0.25, 0, false, 'med', 2500, 2000, 1, 'pumpkin', 'เลื้อยคลุมดิน เก็บง่าย ต้องการแดดและพื้นที่โปร่ง'),
  P('sweetpotato', 'มันเทศ', 'Sweet potato', 'groundcover', 'เถาล้มลุก', 'พืชเลื้อย/หัว', 0, 1600, false, 1, 1, 18, 2200, 0.35, 0, false, 'low', 2500, 2000, 1, 'sweetpotato', 'คลุมดินดี หัวขายได้ ทนแล้ง ปรับตัวได้กว้าง'),
  P('pineapple', 'สับปะรด', 'Pineapple', 'groundcover', 'พืชล้มลุก', 'ไม้ผลล้มลุก', 0, 1000, false, 2, 2, 13.5, 3362, 0.25, 0, false, 'low', 5000, 12000, 1, 'pineapple', 'คลุมดินกันวัชพืช ทนแล้ง เหมาะพื้นที่ร้อนและดินระบายน้ำดี'),

  // ── พืชหัวใต้ดิน (root) ──
  P('ginger', 'ขิง', 'Ginger', 'root', 'พืชล้มลุกมีเหง้า', 'พืชหัว', 300, 1200, false, 1, 1, 30, 2000, 0.6, 0, false, 'high', 8000, 37700, 1, 'ginger', 'ชอบร่มรำไร รายได้ต่อไร่สูง ต้องการความชื้นสม่ำเสมอ'),
  P('turmeric', 'ขมิ้น', 'Turmeric', 'root', 'พืชล้มลุกมีเหง้า', 'พืชหัว/สมุนไพร', 200, 1200, false, 1, 1, 18, 2500, 0.65, 0, false, 'med', 6300, 9600, 1, 'turmeric', 'ทนร่มเงาดีมาก เหมาะใต้ไม้ผล/กล้วย ดินต้องไม่แฉะ'),
  P('taro', 'เผือก', 'Taro', 'root', 'พืชล้มลุกมีหัว', 'พืชหัว', 0, 1200, false, 1, 1, 22, 2500, 0.5, 0, false, 'high', 6000, 10000, 1, 'taro', 'ชอบดินชื้น เหมาะที่ลุ่ม/ร่องน้ำใต้ร่ม ไม่เหมาะแล้งจัด'),
  P('galangal', 'ข่า', 'Galangal', 'root', 'พืชล้มลุกมีเหง้า', 'พืชหัว/เครื่องเทศ', 100, 1200, true, 1, 2, 25, 2000, 0.6, 0, false, 'med', 3000, 2000, 1, 'galangal', 'ทนร่ม ปลูกครั้งเดียวเก็บได้นาน เหมาะต่ำ-กลางถึงพื้นที่สูงไม่หนาวจัด'),
  ...
  P('papaya', 'มะละกอ', 'Papaya', 'canopy', 'พืชล้มลุกขนาดใหญ่', 'ไม้ผล', 0, 800, true, 1, 2, 13, 4000, 0.15, 0.3, false, 'high', 25000, 10000, 1, undefined, 'เก็บผลได้ใน 8 เดือน ใช้เป็นรายได้เร็วระหว่างรอไม้ยืนต้นโต แต่ต้องรื้อปลูกใหม่ทุก 2-3 ปี เพราะโรคใบด่างจุดวงแหวน'),
  ...
  P('somsithong', 'ส้มสีทอง', 'Nan Golden Tangerine', 'canopy', 'ไม้ต้น', 'ไม้ผล', 200, 1200, true, 3, 6, 30, 1021, 0.2, 0.5, false, 'high', 20000, 20900, 1, undefined, 'ต้องมีน้ำรดช่วงแล้ง และใช้กิ่งพันธุ์ปลอดโรคกรีนนิ่งจากสวนส้มในน่าน จึงจะขอใช้ตรา GI ส้มสีทองน่านได้'),
  P('rambutan', 'เงาะ', 'Rambutan', 'canopy', 'ไม้ต้น', 'ผลไม้', 0, 600, true, 4, 8, 20, 900, 0.25, 0.7, false, 'high', 8000, 16300, 1, undefined, 'เงาะปลูกได้เฉพาะที่ราบลุ่มต่ำกว่า 600 ม. และต้องมีน้ำรดตลอดหน้าแล้ง 5 เดือน (พ.ย.-มี.ค.) ถ้าไม่มีระบบน้ำ อย่าปลูก'),
  ...
  P('longkong', 'ลองกอง', 'Longkong', 'shrub', 'ไม้ต้น', 'ไม้ผล', 0, 600, true, 5, 9, 30, 400, 0.65, 0.55, false, 'high', 10000, 10600, 1, undefined, 'ปลูกเฉพาะพื้นที่ต่ำกว่า 600 เมตร ต้องมีน้ำรดช่วงแล้ง 5-6 เดือน และตัดแต่งทรงพุ่มไม่ให้สูงเกิน 6-8 เมตร เพื่อให้อยู่ใต้ไม้เรือนยอด'),
  ...
  P('reao', 'เร่วดง (เร่วใหญ่)', 'Bastard cardamom (Tavoy cardamom)', 'shrub', 'พืชล้มลุกมีเหง้า', 'สมุนไพร/เครื่องเทศ', 300, 1200, true, 3, 5, 400, 25, 0.8, 0.3, false, 'high', 12000, 3000, 1, undefined, 'ปลูกแซมใต้ร่มยางหรือไม้ผลได้โดยไม่ต้องโค่นต้นเดิม แต่ต้องหาผู้รับซื้อก่อนและเริ่มแค่ 1 ไร่ เพราะติดผลยากถ้าไม่มีแมลงผสมเกสร'),
  ...
  // ── new: groundcover ──
  P('calopo', 'ถั่วคาโลโปโกเนียม', 'Calopo', 'groundcover', 'เถาล้มลุก', 'พืชคลุมดิน/ปุ๋ยพืชสด', 0, 1500, false, 1, 1, 0, 0, 0.2, 0.4, true, 'med', 600, 250, 0, undefined, 'ปลูกคลุมดินช่วง 1-4 ปีแรกเพื่อกันดินพังทลายและเพิ่มไนโตรเจน หมั่นตัดเถาที่พันต้นกล้า และจะโรยราเองเมื่อเรือนยอดปิด จึงไม่ใช่พืชถาวร'),
  P('jackbean', 'ถั่วพร้า', 'Jack bean', 'groundcover', 'พืชล้มลุก', 'พืชปุ๋ยสด', 0, 1500, false, 1, 1, 0, 0, 0.5, 0, true, 'low', 1000, 700, 1, undefined, 'หว่าน 10-12 กก./ไร่ ต้นฤดูฝน ไถกลบตอนอายุ 30-40 วัน ได้ไนโตรเจนบำรุงดินแทนปุ๋ยเคมี ปลูกไว้ใช้เอง ไม่ใช่พืชขาย และเมล็ดดิบมีพิษ ห้ามให้คนหรือสัตว์กิน'),
  P('centro', 'ถั่วลาย', 'Centro', 'groundcover', 'เถาล้มลุก', 'พืชคลุมดิน/ปุ๋ยพืชสด', 0, 600, true, 1, 2, 0, 0, 0.8, 0.3, true, 'med', 900, 300, 0, undefined, 'ปลูกคลุมดินใต้ยางหรือไม้ผล ทนร่มได้ถึง 80% ช่วยตรึงไนโตรเจนและกันดินพังบนพื้นที่ลาด แต่ตั้งตัวช้า 4-8 เดือน ต้องดายหญ้าช่วงแรก และไม่มีรายได้ขาย ปลูกเพื่อบำรุงดินเท่านั้น'),
  P('hamata', 'ถั่วฮามาต้า', 'Caribbean stylo', 'groundcover', 'พืชล้มลุก', 'พืชอาหารสัตว์/ปรับปรุงดิน', 0, 1000, true, 0, 1, 0, 300, 0.15, 0.2, true, 'low', 1700, 800, 5, undefined, 'หว่านเมล็ด 2 กก./ไร่ เฉพาะที่แดดจัด เช่น แนวกันไฟหรือช่องว่างระหว่างแถวไม้ในช่วง 1-3 ปีแรก เพราะไม่ทนร่มเงา แล้วตัดเลี้ยงวัวได้ทุก 45-60 วัน'),
  P('mungbean', 'ถั่วเขียว (ผิวมัน)', 'Mungbean', 'groundcover', 'พืชล้มลุก', 'พืชไร่/ถั่วบำรุงดิน', 0, 1000, false, 1, 1, 22, 150, 0.2, 0.2, true, 'low', 0, 2270, 1, undefined, 'ปลูกคลุมดินช่วง 1-3 ปีแรกก่อนไม้ยืนต้นแตกร่ม คลุกเชื้อไรโซเบียมก่อนหยอดเมล็ด และไถกลบต้นหลังเก็บเกี่ยวเพื่อเพิ่มไนโตรเจนให้ดิน'),
  ...
  P('sesame', 'งาดำ', 'Black sesame', 'groundcover', 'พืชล้มลุก', 'พืชน้ำมัน', 0, 900, false, 1, 1, 50, 70, 0.1, 0, false, 'low', 1200, 1200, 1, undefined, 'งาต้องการแดดเต็มที่ ปลูกได้เฉพาะแถวว่างช่วง 1-2 ปีแรกก่อนไม้ใหญ่คลุม และต้องยกร่องระบายน้ำเพราะงาตายง่ายเมื่อน้ำขัง'),
